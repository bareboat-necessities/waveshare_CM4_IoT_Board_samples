...
void DEV_I2C_Init(uint8_t Add)
{
        ...
        bcm2835_i2c_begin();
        printf("BCM2835_I2C_REASON_OK  = %d\r\n",BCM2835_I2C_REASON_OK);
...
void I2C_Write_nByte(uint8_t *value ,uint32_t Lan)
{
	int ref;
#if DEV_I2C
    #ifdef USE_BCM2835_LIB
        bcm2835_i2c_write(value, Lan);
    #elif USE_WIRINGPI_LIB
        //wiringPiI2CWrite(fd,Cmd);
        for(int i = 0;i<Lan;i++){
            wiringPiI2CWrite(fd, value[i]);
        }
    #elif USE_DEV_LIB
        DEV_HARDWARE_I2C_write(value, Lan);
    #endif
#endif
}
...
int I2C_Read_Word(uint8_t Cmd)
{
    ...
    #ifdef USE_BCM2835_LIB
        char rbuf[2] = {0};
        int k = bcm2835_i2c_read_register_rs(&Cmd, rbuf, 2);
        ref = rbuf[1]<<8 | rbuf[0];
...
void I2C_Read_nByte(uint8_t *value ,uint32_t Lan)
{
	int ref;
#if DEV_I2C
    #ifdef USE_BCM2835_LIB
        char rbuf[2] = {0};
        bcm2835_i2c_read(rbuf, Lan);
        ref = rbuf[1]<<8 | rbuf[0];
    ...
    #elif USE_DEV_LIB
        DEV_HARDWARE_I2C_Onlyread(value,Lan);
    #endif
#endif
}
/******************************************************************************
